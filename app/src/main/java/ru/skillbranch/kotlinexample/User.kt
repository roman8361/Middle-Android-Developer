...
class User(
    ...
    constructor(
        ...
        println("Phone passwordHash id $passwordHash")
        accessCode = code
    ...
    init {
        ...
        check(firstName.isNotBlank()) { "FirstName must" }
        check(!email.isNullOrEmpty() || !rawPhone.isNullOrEmpty()) { "Email or phone must not be null or blank" }
                ...
                .run {
                    when (size) {
                        ...
                        2 -> first() to last()
                            ...
                            "FullName must contain only first name ande last name, " +
